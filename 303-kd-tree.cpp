...
struct kd_tree { int k, size; struct point {
    int data[MAXK], id; } p[MAXN];
  struct kd_node { int l, r; point p, dmin, dmax;
    kd_node() {} kd_node(const point &rhs)
        : l(-1), r(-1), p(rhs), dmin(rhs),
          dmax(rhs) {}
    ...
    long long min_dist(
      const point &rhs, int k) const {
      register long long ret = 0;
      for (register int i = 0; i < k; ++i) {
        if (dmin.data[i] <= rhs.data[i] &&
          rhs.data[i] <= dmax.data[i]) continue;
        ret += std::min(1ll *
    ...
    long long max_dist(const point &rhs, int k) {
      ...
      for (int i = 0; i < k; ++i) {
        int tmp = std::max(
          std::abs(dmin.data[i] - rhs.data[i]),
          std::abs(dmax.data[i] - rhs.data[i]));
        ret += 1ll * tmp * tmp; }
      // ret += std::max (std::abs
      //(rhs.data[i] - dmax.data[i]) + std::abs
      //(rhs.data[i] - dmin.data[i])); }
      return ret; } } tree[MAXN * 4];
  struct result { long long dist; point d;
    result() {}
  ...
  long long sqrdist(
    ...
    long long ret = 0; for (int i = 0; i < k; ++i)
  ...
  int alloc() { tree[size].l = tree[size].r = -1;
    return size++; }
  void build(const int &depth, int &rt,
    ...
    if (l == r) return; build(
      (depth + 1) % k, tree[rt].l, l, middle - 1);
    build(
      (depth + 1) % k, tree[rt].r, middle + 1, r);
    if (~tree[rt].l)
      tree[rt].merge(tree[tree[rt].l], k);
    if (~tree[rt].r)
      tree[rt].merge(tree[tree[rt].r], k); }
  std::priority_queue<result, std::vector<result>,
    std::less<result>> heap_l;
  std::priority_queue<result, std::vector<result>,
    std::greater<result>> heap_r;
  void _min_kth(const int &depth, const int &rt,
    const int &m, const point &d) { result tmp =
      result(sqrdist(tree[rt].p, d), tree[rt].p);
    if ((int)heap_l.size() < m) heap_l.push(tmp);
    else if (tmp < heap_l.top()) { heap_l.pop();
      heap_l.push(tmp); }
    int x = tree[rt].l, y = tree[rt].r;
    if (~x && ~y &&
      sqrdist(d, tree[x].p) > sqrdist(d, tree[y].p))
      std::swap(x, y);
    if (~x && ((int)heap_l.size() < m ||
        tree[x].min_dist(d, k) < heap_l.top().dist))
      _min_kth((depth + 1) % k, x, m, d);
    if (~y && ((int)heap_l.size() < m ||
  ...
  void _max_kth(const int &depth, const int &rt,
    ...
    else if (tmp > heap_r.top()) { heap_r.pop();
      heap_r.push(tmp); }
    int x = tree[rt].l, y = tree[rt].r;
    if (~x && ~y &&
      sqrdist(d, tree[x].p) < sqrdist(d, tree[y].p))
      std::swap(x, y);
    if (~x && ((int)heap_r.size() < m ||
        tree[x].max_dist(d, k) >=
          heap_r.top().dist))
      _max_kth((depth + 1) % k, x, m, d);
    if (~y && ((int)heap_r.size() < m ||
        tree[y].max_dist(d, k) >=
          heap_r.top().dist))
  ...
  result min_kth(const point &d, const int &m) {
    ...
    _min_kth(0, 0, m, d); return heap_l.top(); }
  result max_kth(const point &d, const int &m) {
    heap_r = decltype(heap_r)();
    _max_kth(0, 0, m, d); return heap_r.top(); } };
