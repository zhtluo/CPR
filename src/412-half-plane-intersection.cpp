std::vector<point> cut(
  ...
  std::vector<point> ret; if (c.empty()) return ret;
  for (int i = 0; i < (int)c.size(); ++i) {
    int j = (i + 1) % (int)c.size();
    if (turn_left(p.s, p.t, c[i]))
      ret.push_back(c[i]);
    if (two_side(c[i], c[j], p)) ret.push_back(
        line_intersect(p, line(c[i], c[j]))); }
  return ret; }
bool turn_left(cl l, cp p) {
  return sgn(det(l.t - l.s, p - l.s)) >= 0; }
int cmp(cp a, cp b) { return a.dim() != b.dim()
    ? (a.dim() < b.dim() ? -1 : 1)
    : -sgn(det(a, b)); }
