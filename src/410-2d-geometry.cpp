...
double point_to_line(cp a, cl b) {
  return std::abs(det(b.t - b.s, a - b.s)) /
    dis(b.s, b.t); }
point project_to_line(cp a, cl b) { return b.s +
    (b.t - b.s) *
...
std::vector<point> line_circle_intersect(
  ...
  double x =
    msqrt(sqr(b.r) - sqr(point_to_line(b.c, a)));
  point s = project_to_line(b.c, a),
        u = (a.t - a.s).unit();
  ...
  if (sgn(d - std::abs(a.r - b.r)) <= 0) {
    ...
    return r * r * PI; }
  double x = (d * d + a.r * a.r - b.r * b.r) /
    (2 * d), t1 = acos(
           std::min(1., std::max(-1., x / a.r))),
         t2 = acos(std::min(
           1., std::max(-1., (d - x) / b.r)));
...
std::vector<point> circle_intersect(cc a, cc b) {
  ...
  if (sgn(h) == 0)
    return std::vector<point>({a.c + r * x});
  return std::vector<point>(
    {a.c + r * x - r.rot90() * h,
      a.c + r * x + r.rot90() * h}); }
...
std::vector<line> extangent(cc a, cc b) {
  ...
  if (cmp(dis(a.c, b.c), std::abs(a.r - b.r)) <= 0)
    return ret;
  if (sgn(a.r - b.r) == 0) { point dir = b.c - a.c;
